...
namespace core
{
    ...
    struct matrix3
    {
        ...
        matrix3(const Vec3<T>& row1, const Vec3<T>& row2, const Vec3<T>& row3) : rows{{row1, row2, row3}} {}
        ...
        [[nodiscard]] constexpr T determinant() const
        {
            ...
            // along the first row
            return rows[0].x * core::matrix2<T>(Vec2<T>(rows[1].y, rows[1].z), Vec2<T>(rows[2].y, rows[2].z)).determinant()
                   - rows[0].y * core::matrix2<T>(Vec2<T>(rows[1].x, rows[1].z), Vec2<T>(rows[2].x, rows[2].z)).determinant()
                   + rows[0].z * core::matrix2<T>(Vec2<T>(rows[1].x, rows[1].y), Vec2<T>(rows[2].x, rows[2].y)).determinant();
        }
